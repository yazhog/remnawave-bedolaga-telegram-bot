...
(ТЕСТИРУЕТСЯ, МОЖЕТ НЕ РАБОТАТЬ)

Для быстрой установки и настройки бота используйте автоматический установщик:

## Установка

```bash
# Скачайте установщик
wget https://raw.githubusercontent.com/Fr1ngg/remnawave-bedolaga-telegram-bot/main/bedolaga.sh
...
- ✅ **Автоматически находит и настраивает веб-серверы** (Nginx/Caddy)
- ✅ Создает systemd службу для автозапуска

### 🎛️ Меню управления
После установки доступно удобное меню:

```bash
sudo ./bedolaga.sh --manage
```
...
### 🌐 Автонастройка веб-сервера

Установщик **автоматически обнаружит** ваш веб-сервер (Nginx или Caddy) и настроит webhook для Tribute:

- Поддерживает системные и Docker-версии
- Создает резервные копии конфигураций
- Добавляет правила для `https://ваш-домен.com/tribute-webhook`
- Настраивает health-check эндпоинт

## Команды

| Команда | Описание |
|---------|----------|
| `sudo ./bedolaga.sh` | Запуск установки |
| `sudo ./bedolaga.sh --manage` | Меню управления |
| `sudo ./bedolaga.sh --help` | Справка |

## Интерактивная настройка

Установщик проведет вас через настройку всех параметров:

- 🤖 **Основные настройки бота** (токен, username)
- 👥 **Реферальная система** (награды, проценты)
- 🌐 **RemnaWave API** (URL панели, токен)
- 🆓 **Триал подписки** (срок, лимиты, уведомления)
- ⭐ **Telegram Stars** (курсы обмена)
- 🎁 **Tribute донаты** (API ключ, ссылка)
- 👑 **Администраторы** и поддержка
- ⚙️ **Мониторинг** и автоматизация
