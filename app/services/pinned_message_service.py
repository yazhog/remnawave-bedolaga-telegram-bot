...
async def deliver_pinned_message_to_user(
    ...
    sent = await _send_and_pin_message(bot, user.telegram_id, pinned_message)
    if sent:
        await db.execute(
            update(User)
            .where(User.id == user.id)
            .values(
                last_pinned_message_id=pinned_message.id,
...
async def broadcast_pinned_message(
    ...
    async def send_to_user(user: User) -> Optional[int]:
    ...
    for i in range(0, len(users), 30):
        ...
        success_ids = [user_id for user_id in results if user_id]
        if success_ids:
            await db.execute(
                update(User)
                .where(User.id.in_(success_ids))
                .values(
                    last_pinned_message_id=pinned_message.id,
                    updated_at=datetime.utcnow(),
                )
