    ...
    *,
    ...
    for item in items:
        rows.append([
            InlineKeyboardButton(
                text=item.get("text", "—"),
                callback_data=item.get("callback", "admin_transactions"),
            )
        ])
    ...
    if total_pages > 1:
        ...
        if current_page > 1:
            pagination_row.append(
                InlineKeyboardButton(
                    text="⬅️",
                    callback_data=f"admin_transactions_page_{current_page - 1}",
                )
            )
        pagination_row.append(
            InlineKeyboardButton(
                text=f"{current_page}/{total_pages}",
                callback_data=f"admin_transactions_page_{current_page}",
            )
        )
        if current_page < total_pages:
            pagination_row.append(
    ...
    rows.append([
        InlineKeyboardButton(text=texts.ADMIN_BACK_TO_ADMIN, callback_data="admin_panel")
    ])

    return InlineKeyboardMarkup(inline_keyboard=rows)


def get_admin_transaction_view_keyboard(
    *,
    language: str = DEFAULT_LANGUAGE,
    method: str,
    local_id: int,
    ...
    if can_manual_check:
        rows.append([
            InlineKeyboardButton(
                text=texts.t("ADMIN_TRANSACTION_MANUAL_CHECK_BUTTON", "🔄 Проверить"),
                callback_data=f"admin_tx_check_{method}_{local_id}_p{page}",
            )
        ])
    ...
    if user_id:
        rows.append([
            InlineKeyboardButton(
                text=texts.t("ADMIN_TRANSACTION_GO_TO_USER", "👤 К пользователю"),
    ...
    rows.append([
        InlineKeyboardButton(
            text=texts.t("ADMIN_TRANSACTION_BACK_BUTTON", "⬅️ К списку"),
            callback_data=f"admin_transactions_page_{page}",
        )
    ])
