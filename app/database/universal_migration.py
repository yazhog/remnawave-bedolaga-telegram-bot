...
async def create_promo_offers_table():
    table_exists = await check_table_exists('promo_offers')
    if table_exists:
        logger.info("Таблица promo_offers уже существует")
        return True
    ...
    try:
            ...
            if db_type == 'sqlite':
                await conn.execute(text("""
                    CREATE TABLE promo_offers (
                        ...
                        button_text VARCHAR(100) NOT NULL DEFAULT 'Активировать',
                        ...
                        discount_percent INTEGER NOT NULL DEFAULT 0,
                        bonus_amount_kopeks INTEGER NOT NULL DEFAULT 0,
                        discount_valid_hours INTEGER NOT NULL DEFAULT 0,
                        test_access_hours INTEGER NOT NULL DEFAULT 0,
                        test_squad_uuids TEXT NOT NULL DEFAULT '[]',
                ...
                await conn.execute(text("""
                    CREATE TABLE IF NOT EXISTS promo_offers (
                        ...
                        discount_valid_hours INTEGER NOT NULL DEFAULT 0,
                        test_access_hours INTEGER NOT NULL DEFAULT 0,
                        test_squad_uuids JSONB NOT NULL DEFAULT '[]'::jsonb,
                        created_by INTEGER NULL,
                ...
                await conn.execute(text("""
                    CREATE TABLE IF NOT EXISTS promo_offers (
                        ...
                        discount_valid_hours INTEGER NOT NULL DEFAULT 0,
                        test_access_hours INTEGER NOT NULL DEFAULT 0,
                        test_squad_uuids JSON NOT NULL DEFAULT ('[]'),
                        created_by INTEGER NULL,
                        is_cancelled BOOLEAN NOT NULL DEFAULT FALSE,
                        status VARCHAR(30) NOT NULL DEFAULT 'scheduled',
                        total_count INTEGER NOT NULL DEFAULT 0,
        ...
        logger.info("✅ Таблица promo_offers успешно создана")
        return True

    except Exception as e:
        logger.error(f"Ошибка создания таблицы promo_offers: {e}")
        return False


async def create_promo_offer_deliveries_table():
    table_exists = await check_table_exists('promo_offer_deliveries')
    if table_exists:
        logger.info("Таблица promo_offer_deliveries уже существует")
        return True
    ...
    try:
        async with engine.begin() as conn:
            ...
            if db_type == 'sqlite':
                await conn.execute(text("""
                    CREATE TABLE promo_offer_deliveries (
                        id INTEGER PRIMARY KEY AUTOINCREMENT,
                        offer_id INTEGER NOT NULL,
                        user_id INTEGER NOT NULL,
                        discount_offer_id INTEGER NULL,
                        ...
                        FOREIGN KEY(offer_id) REFERENCES promo_offers(id) ON DELETE CASCADE,
                        FOREIGN KEY(user_id) REFERENCES users(id) ON DELETE CASCADE,
                        FOREIGN KEY(discount_offer_id) REFERENCES discount_offers(id) ON DELETE SET NULL
                    )
                """))

            elif db_type == 'postgresql':
                await conn.execute(text("""
                    CREATE TABLE IF NOT EXISTS promo_offer_deliveries (
                        id SERIAL PRIMARY KEY,
                        offer_id INTEGER NOT NULL REFERENCES promo_offers(id) ON DELETE CASCADE,
                        user_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,
                        discount_offer_id INTEGER NULL REFERENCES discount_offers(id) ON DELETE SET NULL,
                        status VARCHAR(20) NOT NULL DEFAULT 'sent',
                        error_message TEXT NULL,
                        sent_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                        activated_at TIMESTAMP NULL,
                        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                        CONSTRAINT uq_promo_offer_deliveries_offer_user UNIQUE (offer_id, user_id)
                    )
                """))
                ...
                await conn.execute(text("""
                    CREATE TABLE IF NOT EXISTS promo_offer_deliveries (
                        ...
                        created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
                        CONSTRAINT uq_promo_offer_deliveries_offer_user UNIQUE (offer_id, user_id),
                        CONSTRAINT fk_promo_offer_delivery_offer FOREIGN KEY(offer_id) REFERENCES promo_offers(id) ON DELETE CASCADE,
                        CONSTRAINT fk_promo_offer_delivery_user FOREIGN KEY(user_id) REFERENCES users(id) ON DELETE CASCADE,
                        CONSTRAINT fk_promo_offer_delivery_discount FOREIGN KEY(discount_offer_id) REFERENCES discount_offers(id) ON DELETE SET NULL
                    )
                """))
        ...
        logger.info("✅ Таблица promo_offer_deliveries успешно создана")
        return True

    except Exception as e:
        logger.error(f"Ошибка создания таблицы promo_offer_deliveries: {e}")
        return False


async def create_promo_offer_activations_table():
    table_exists = await check_table_exists('promo_offer_activations')
    if table_exists:
        logger.info("Таблица promo_offer_activations уже существует")
        return True
    ...
    try:
        async with engine.begin() as conn:
            ...
            if db_type == 'sqlite':
                await conn.execute(text("""
                    CREATE TABLE promo_offer_activations (
                        id INTEGER PRIMARY KEY AUTOINCREMENT,
                        offer_id INTEGER NOT NULL,
                        user_id INTEGER NOT NULL,
                        subscription_id INTEGER NULL,
                ...
                await conn.execute(text("""
                    CREATE TABLE IF NOT EXISTS promo_offer_activations (
                        id SERIAL PRIMARY KEY,
                        offer_id INTEGER NOT NULL REFERENCES promo_offers(id) ON DELETE CASCADE,
                        user_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,
                        subscription_id INTEGER NULL REFERENCES subscriptions(id) ON DELETE SET NULL,
                        ...
                        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                        CONSTRAINT uq_promo_offer_activations_offer_user UNIQUE (offer_id, user_id)
                    )
                """))
                ...
                await conn.execute(text("""
                    CREATE TABLE IF NOT EXISTS promo_offer_activations (
                        ...
                        created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
                        CONSTRAINT uq_promo_offer_activations_offer_user UNIQUE (offer_id, user_id),
                        CONSTRAINT fk_promo_offer_activation_offer FOREIGN KEY(offer_id) REFERENCES promo_offers(id) ON DELETE CASCADE,
                        CONSTRAINT fk_promo_offer_activation_user FOREIGN KEY(user_id) REFERENCES users(id) ON DELETE CASCADE,
                        CONSTRAINT fk_promo_offer_activation_subscription FOREIGN KEY(subscription_id) REFERENCES subscriptions(id) ON DELETE SET NULL,
                        CONSTRAINT fk_promo_offer_activation_discount FOREIGN KEY(discount_offer_id) REFERENCES discount_offers(id) ON DELETE SET NULL
                    )
                """))
        ...
        logger.info("✅ Таблица promo_offer_activations успешно создана")
        return True

    except Exception as e:
        logger.error(f"Ошибка создания таблицы promo_offer_activations: {e}")
        return False
...
async def run_universal_migration():
    ...
    try:
        ...
        logger.info("=== СОЗДАНИЕ ТАБЛИЦЫ PROMO_OFFERS ===")
        promo_offers_created = await create_promo_offers_table()
        if promo_offers_created:
            logger.info("✅ Таблица promo_offers готова")
        else:
            logger.warning("⚠️ Проблемы с таблицей promo_offers")

        logger.info("=== СОЗДАНИЕ ТАБЛИЦЫ PROMO_OFFER_DELIVERIES ===")
        promo_deliveries_created = await create_promo_offer_deliveries_table()
        if promo_deliveries_created:
            logger.info("✅ Таблица promo_offer_deliveries готова")
        else:
            logger.warning("⚠️ Проблемы с таблицей promo_offer_deliveries")

        logger.info("=== СОЗДАНИЕ ТАБЛИЦЫ PROMO_OFFER_ACTIVATIONS ===")
        promo_activations_created = await create_promo_offer_activations_table()
        if promo_activations_created:
            logger.info("✅ Таблица promo_offer_activations готова")
        else:
            logger.warning("⚠️ Проблемы с таблицей promo_offer_activations")
