...
async def ensure_user_last_pinned_column():
    table_exists = await check_table_exists("users")
    if not table_exists:
        logger.warning("⚠️ Таблица users отсутствует — пропускаем колонку last_pinned_message_id")
        return False
    ...
    try:
        async with engine.begin() as conn:
            ...
            if not await check_column_exists("users", "last_pinned_message_id"):
                await conn.execute(
                    text("ALTER TABLE users ADD COLUMN last_pinned_message_id INTEGER NULL")
                )
                ...
                if db_type == "postgresql":
                    fk_sql = (
                        "ALTER TABLE users "
                        "ADD CONSTRAINT fk_users_last_pinned_message "
                        "FOREIGN KEY (last_pinned_message_id) REFERENCES pinned_messages(id) ON DELETE SET NULL"
                    )
                elif db_type == "mysql":
                    fk_sql = (
                        "ALTER TABLE users "
        ...
        logger.info("✅ Колонка last_pinned_message_id в users приведена в актуальное состояние")
        return True

    except Exception as e:
        logger.error(f"Ошибка добавления last_pinned_message_id в users: {e}")
        return False
...
async def run_universal_migration():
    ...
    try:
        ...
        logger.info("=== ДОБАВЛЕНИЕ ТРЕКЕРА ДОСТАВКИ ЗАКРЕПЛЕННОГО СООБЩЕНИЯ ДЛЯ ПОЛЬЗОВАТЕЛЕЙ ===")
        user_pinned_tracker_ready = await ensure_user_last_pinned_column()
        if user_pinned_tracker_ready:
            logger.info("✅ Колонка last_pinned_message_id в users готова")
        else:
            logger.warning("⚠️ Проблемы с добавлением last_pinned_message_id в users")
...
async def check_migration_status():
    ...
    try:
        status = {
            ...
            "pinned_messages_frequency_column": False,
        ...
        status["pinned_messages_frequency_column"] = (
            status["pinned_messages_table"]
        ...
        check_names = {
            ...
            "pinned_messages_frequency_column": "Режим отправки закрепа (/start/обновление)",
            "users_last_pinned_column": "Трекер последнего закрепа у пользователей",
