...
def _build_balance_payload(user_id: int, amount_kopeks: int) -> str:
    unique_suffix = uuid.uuid4().hex
    return f"balance_{user_id}_{amount_kopeks}_{unique_suffix}"
...
async def _find_recent_deposit(
    ...
    *,
    ...
    if invoice_payload:
        query = query.where(Transaction.description.contains(invoice_payload))
...
async def create_payment_link(
    ...
    if method == "stars":
        ...
        invoice_payload = _build_balance_payload(user.id, amount_kopeks)
        try:
    ...
    if method == "cryptobot":
        ...
        payment_service = PaymentService()
        cryptobot_payload = _build_balance_payload(user.id, amount_kopeks)
        ...
        return MiniAppPaymentCreateResponse(
            ...
            extra={
                ...
                "amount_usd": amount_usd,
                "rate": rate,
                "payload": result.get("payload") or cryptobot_payload,
                "requested_at": _current_request_timestamp(),
            },
        )
