...
from ..schemas.miniapp import (
    ...
    MiniAppSubscriptionUpdateConfirmation,
)
    ...
    if not subscription.is_active:
        raise HTTPException(
            ...
            detail={
                ...
                "message": "Subscription must be active to manage paid options",
            },
        )
...
def _localized_text(ru: Optional[str], en: Optional[str]) -> Dict[str, str]:
...
def _format_months_label(months: int, language: str) -> str:
...
def _build_confirmation_from_lines(
    ru_lines: List[str],
    en_lines: List[str],
    *,
    ru_title: str = "Подтвердите изменение",
    en_title: str = "Confirm change",
) -> MiniAppSubscriptionUpdateConfirmation:
    ru_message = "\n".join(line for line in ru_lines if line)
    en_message = "\n".join(line for line in en_lines if line)
    return MiniAppSubscriptionUpdateConfirmation(
        title=_localized_text(ru_title, en_title),
        message=_localized_text(ru_message, en_message),
        confirm_label=_localized_text("Подтвердить", "Confirm"),
        cancel_label=_localized_text("Отмена", "Cancel"),
    )


def _build_servers_confirmation(
    ...
    if added_names:
        ...
        ru_lines.append(f"Будут подключены серверы: {joined}.")
        en_lines.append(f"The following servers will be added: {joined}.")

    if removed_names:
        joined = ", ".join(removed_names)
        ru_lines.append(
            f"Будут отключены серверы: {joined}. Возврат средств не предусмотрен."
        )
        en_lines.append(
            f"The following servers will be removed: {joined}. No refunds are provided."
        )

    if total_cost > 0:
        price_label = settings.format_price(total_cost)
        months_ru = _format_months_label(charged_months, "ru") if charged_months else None
        months_en = _format_months_label(charged_months, "en") if charged_months else None
        if months_ru:
            ru_lines.append(
                f"С вашего баланса будет списано {price_label} за {months_ru}. Продолжить?"
            )
        else:
            ru_lines.append(
                f"С вашего баланса будет списано {price_label}. Продолжить?"
            )
        if months_en:
            en_lines.append(
                f"{price_label} will be charged for {months_en}. Continue?"
            )
        else:
            en_lines.append(f"{price_label} will be charged. Continue?")
    else:
        if added_names or removed_names:
            ru_lines.append("Изменение будет бесплатным. Продолжить?")
            en_lines.append("The change is free. Continue?")
        else:
            ru_lines.append("Подтвердите изменение списка серверов.")
            en_lines.append("Confirm the server list update.")

    return _build_confirmation_from_lines(ru_lines, en_lines)


def _format_traffic_label_ru(value: Optional[int]) -> str:
    if not value:
        return "Безлимит"
    return f"{value} ГБ"


def _format_traffic_label_en(value: Optional[int]) -> str:
    if not value:
        return "Unlimited"
    return f"{value} GB"


def _build_traffic_confirmation(
    current_value: Optional[int],
    new_value: Optional[int],
    price_to_charge: int,
    charged_months: int,
) -> MiniAppSubscriptionUpdateConfirmation:
    ru_current = _format_traffic_label_ru(current_value)
    ru_new = _format_traffic_label_ru(new_value)
    en_current = _format_traffic_label_en(current_value)
    en_new = _format_traffic_label_en(new_value)

    ru_lines = [f"Лимит изменится с {ru_current} до {ru_new}."]
    en_lines = [f"Traffic limit will change from {en_current} to {en_new}."]

    if price_to_charge > 0:
        price_label = settings.format_price(price_to_charge)
        months_ru = _format_months_label(charged_months, "ru") if charged_months else None
        months_en = _format_months_label(charged_months, "en") if charged_months else None
        if months_ru:
            ru_lines.append(
...
def _build_devices_confirmation(
    current_devices: int,
    new_devices: int,
    price_to_charge: int,
    charged_months: int,
    ...
    ru_lines = [
        f"Количество устройств изменится с {current_devices} до {new_devices}."
    ]
    en_lines = [
        f"Device limit will change from {current_devices} to {new_devices}."
    ]
...
async def update_subscription_servers_endpoint(
    ...
    added_names_for_confirmation = [catalog[uuid].get("name", uuid) for uuid in added]
    removed_names_for_confirmation = [
        catalog.get(uuid, {}).get("name", uuid)
        for uuid in removed
    ]

    if not bool(payload.confirm):
        confirmation = _build_servers_confirmation(
            added_names_for_confirmation,
            removed_names_for_confirmation,
            total_cost,
            charged_months,
        )
    ...
    if total_cost > 0:
        added_names = added_names_for_confirmation
        description = (
...
async def update_subscription_traffic_endpoint(
    ...
    if not bool(payload.confirm):
        confirmation = _build_traffic_confirmation(
...
async def update_subscription_devices_endpoint(
    ...
    months_for_confirmation = charged_months or get_remaining_months(subscription.end_date)

    if not bool(payload.confirm):
        confirmation = _build_devices_confirmation(
            current_devices,
            new_devices,
            price_to_charge,
            months_for_confirmation,
        )
        return MiniAppSubscriptionUpdateResponse(
            success=False,
            confirmation_required=True,
    ...
    if price_to_charge > 0:
        ...
        await create_transaction(
            ...
            description=f"{description} на {months_for_confirmation} мес",
        )
