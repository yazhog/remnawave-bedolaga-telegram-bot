...
async def _calculate_purchase_summary(
    ...
    identifier = str(period.id or period.code or period.period_days or selection.get("period_id") or "")
    if identifier:
        selection["period_id"] = identifier

    period_days_value: Optional[int] = None
    if period.period_days is not None:
        try:
            period_days_value = int(period.period_days)
        except (TypeError, ValueError):
            period_days_value = None

    if period_days_value is None:
        try:
            raw_days = selection.get("period_days")
            period_days_value = int(raw_days) if raw_days is not None else None
        except (TypeError, ValueError):
            period_days_value = None

    if period_days_value is None:
        try:
            months_hint = period.period_months or period.months or selection.get("period_months")
            if months_hint is not None:
                period_days_value = int(months_hint) * 30
        except (TypeError, ValueError):
            period_days_value = None
    ...
    selection["period_days"] = period_days_value

    months_hint: Optional[int] = None
    try:
        months_hint = (
            int(period.period_months)
            if period.period_months is not None
            else int(period.months) if period.months is not None else None
        )
    except (TypeError, ValueError):
        months_hint = None

    if months_hint is None:
        try:
            raw_months = selection.get("period_months")
            months_hint = int(raw_months) if raw_months is not None else None
        except (TypeError, ValueError):
            months_hint = None

    months = months_hint or calculate_months_from_days(period_days_value)
    months = max(1, months)
