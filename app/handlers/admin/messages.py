...
async def show_pinned_message_menu(
    ...
    if pinned_message:
        ...
        frequency_line = (
            "🔁 Показывать при каждом /start"
            if pinned_message.send_on_every_start
            else "⏱️ Показывать один раз и при обновлении"
        )
        body = (
...
@admin_required
@error_handler
async def toggle_pinned_message_frequency(
    callback: types.CallbackQuery,
    db_user: User,
    db: AsyncSession,
    state: FSMContext,
...
def register_handlers(dp: Dispatcher):
    ...
    dp.callback_query.register(toggle_pinned_message_frequency, F.data == "admin_pinned_message_frequency")
