...
def _normalize_hours_value(raw: Optional[object]) -> Optional[float]:
    if raw is None:
        return None
...
def _format_hours_value(hours: float) -> str:
    try:
        value = float(hours)
    except (TypeError, ValueError):
        return str(hours)
...
def _extract_active_hours_from_offer(offer) -> Optional[float]:
    if not offer:
        return None
    ...
    for key in ("active_discount_hours", "active_hours", "duration_hours"):
        value = _normalize_hours_value(extra_data.get(key))
...
def _format_offer_time_left(delta: timedelta, texts) -> str:
    seconds = int(delta.total_seconds())
    if seconds <= 0:
        return texts.t("ADMIN_PROMO_OFFER_TIME_LEFT_EXPIRED", "истекло")

    minutes_total = max(1, (seconds + 59) // 60)
    days, remainder_minutes = divmod(minutes_total, 60 * 24)
    ...
    if days:
        parts.append(
            texts.t(
                "ADMIN_PROMO_OFFER_TIME_LEFT_DAYS",
                "{count} дн.",
            ).format(count=days)
        )
    ...
    if hours:
        parts.append(
            texts.t(
                "ADMIN_PROMO_OFFER_TIME_LEFT_HOURS",
                "{count} ч.",
            ).format(count=hours)
        )
    ...
    if minutes and not days:
        parts.append(
            texts.t(
                "ADMIN_PROMO_OFFER_TIME_LEFT_MINUTES",
                "{count} мин.",
            ).format(count=minutes)
        )

    if not parts:
        parts.append(texts.t("ADMIN_PROMO_OFFER_TIME_LEFT_LESS_THAN_MINUTE", "<1 мин."))

    return " ".join(parts)


def _resolve_offer_status_label(offer, texts, *, now: Optional[datetime] = None) -> str:
    reference = now or datetime.utcnow()

    if getattr(offer, "claimed_at", None):
        return texts.t("ADMIN_PROMO_OFFER_STATUS_ACCEPTED", "✅ Принято")

    expires_at = getattr(offer, "expires_at", None)
    is_active = getattr(offer, "is_active", False)
    if is_active and (expires_at is None or expires_at > reference):
        return texts.t("ADMIN_PROMO_OFFER_STATUS_PENDING", "🕒 Не принято")

    return texts.t("ADMIN_PROMO_OFFER_STATUS_EXPIRED", "⌛ Истекло")
...
async def _render_send_user_list(
    *,
    ...
    latest_offer_candidates = await list_discount_offers(
        db,
        user_id=user.id,
        notification_type=notification_type,
        limit=1,
    )
    latest_offer = latest_offer_candidates[0] if latest_offer_candidates else None
    total_template_offers = await count_discount_offers(
        db,
        user_id=user.id,
        notification_type=notification_type,
    )
...
async def show_selected_user_details(
    ...
    if subscription:
        ...
        lines.append(
            texts.t(
                ...
                "Истекает: {date}",
            ).format(date=format_datetime(subscription.end_date, db_user.language))
        )
        lines.append(
            texts.t(
    ...
    if percent > 0 and (not expires_at or expires_at > now):
        ...
        if expires_at:
            discount_line += texts.t(
                ...
                " (до {date})",
            ).format(date=format_datetime(expires_at, db_user.language))
    ...
    lines.append(
        texts.t(
            ...
            "Действует: {hours} ч.",
        ).format(hours=_format_hours_value(template.valid_hours or 0))
    )

    latest_offer_active_hours = _extract_active_hours_from_offer(latest_offer)
    template_active_hours = _normalize_hours_value(template.active_discount_hours)
    effective_active_hours = latest_offer_active_hours or template_active_hours

    if latest_offer:
        status_label = _resolve_offer_status_label(latest_offer, texts, now=now)
    else:
        status_label = texts.t("ADMIN_PROMO_OFFER_STATUS_NOT_SENT", "📭 Не отправлялось")

    lines.append(
        texts.t(
            "ADMIN_PROMO_OFFER_SEND_USER_TEMPLATE_STATUS",
            "Статус: {status}",
        ).format(status=status_label)
    )

    if latest_offer and latest_offer.claimed_at:
        lines.append(
            texts.t(
                "ADMIN_PROMO_OFFER_SEND_USER_TEMPLATE_ACCEPTED_AT",
                "Принято: {date}",
            ).format(date=format_datetime(latest_offer.claimed_at, db_user.language))
        )
    elif latest_offer and latest_offer.expires_at:
        time_left_text = _format_offer_time_left(latest_offer.expires_at - now, texts)
        lines.append(
            texts.t(
                "ADMIN_PROMO_OFFER_SEND_USER_TEMPLATE_TIME_LEFT",
                "Осталось: {time_left}",
            ).format(time_left=time_left_text)
        )
    ...
    if template.offer_type == "test_access":
        ...
        active_hours = effective_active_hours or 0
        if active_hours:
            lines.append(
                texts.t(
                    "ADMIN_PROMO_OFFER_SEND_USER_TEMPLATE_ACTIVE_DURATION",
                    "После активации действует {hours} ч.",
                ).format(hours=_format_hours_value(active_hours))
            )

    lines.append(
        texts.t(
            "ADMIN_PROMO_OFFER_SEND_USER_TEMPLATE_TOTAL_SENT",
            "Всего отправлено: {count}",
        ).format(count=total_template_offers)
    )
    ...
    if active_offers:
        ...
        for offer in active_offers[:5]:
            ...
            expires_text = (
                format_datetime(offer.expires_at, db_user.language)
                if offer.expires_at
                else texts.t("ADMIN_PROMO_OFFER_SEND_USER_OFFER_NO_EXPIRY", "без срока")
            )
            status_label = _resolve_offer_status_label(offer, texts, now=now)
            detail_lines = [
                texts.t(
            ...
            if offer.expires_at:
                time_left_text = _format_offer_time_left(offer.expires_at - now, texts)
                detail_lines.append(
                    texts.t(
                        "ADMIN_PROMO_OFFER_SEND_USER_OFFER_TIME_LEFT",
                        "Осталось: {time}",
                    ).format(time=time_left_text)
                )

            active_offer_hours = _extract_active_hours_from_offer(offer)
            if active_offer_hours:
                detail_lines.append(
                    texts.t(
                        "ADMIN_PROMO_OFFER_SEND_USER_OFFER_ACTIVE_DURATION",
                        "После активации: {hours} ч.",
                    ).format(hours=_format_hours_value(active_offer_hours))
                )
            ...
            if detail_lines:
                ...
                lines.append(
                    texts.t(
                        "ADMIN_PROMO_OFFER_SEND_USER_OFFER_ITEM_WITH_DETAILS",
                        "• {description} (до {expires})\n  {details}",
                    ).format(description=description, expires=expires_text, details=details_text)
                )
            else:
                lines.append(
                    texts.t(
                        "ADMIN_PROMO_OFFER_SEND_USER_OFFER_ITEM",
                        "• {description} (до {expires})",
                    ).format(description=description, expires=expires_text)
                )
    else:
        lines.append("")
        lines.append(
    ...
    if accesses:
        ...
        for entry in accesses[:5]:
            ...
            expires_text = (
                format_datetime(entry.expires_at, db_user.language)
                if entry.expires_at
                else texts.t("ADMIN_PROMO_OFFER_SEND_USER_OFFER_NO_EXPIRY", "без срока")
            )
            lines.append(
