    ...
    *,
    ...
    if record.method == PaymentMethod.CRYPTOBOT:
        ...
        if crypto_amount and crypto_asset:
            lines.append(
                f"   💱 {html.escape(str(crypto_amount))} {html.escape(str(crypto_asset))}"
            )
...
def _build_payment_details_text(record: PendingPayment, *, texts, language: str) -> str:
    ...
    if record.method == PaymentMethod.CRYPTOBOT:
        ...
        if getattr(payment, "paid_at", None):
            ...
            lines.append(
                f"💸 {texts.t('ADMIN_PAYMENT_PAID_AT', 'Paid at')}: {paid_at}"
            )
    ...
    if record.method == PaymentMethod.TELEGRAM_STARS:
        if getattr(payment, "completed_at", None):
            lines.append(
                f"✅ {texts.t('ADMIN_PAYMENT_COMPLETED_AT', 'Completed at')}: "
                f"{format_datetime(payment.completed_at)}"
            )
