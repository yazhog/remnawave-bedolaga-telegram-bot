...
def _build_record_lines(
    ...
    *,
    ...
    amount = record.amount_display or settings.format_price(record.amount_kopeks)
...
def _build_payment_details_text(record: PendingPayment, *, texts, language: str) -> str:
    ...
    if record.method == PaymentMethod.CRYPTOBOT:
        if getattr(payment, "asset", None):
            lines.append(
                f"🪙 {texts.t('ADMIN_PAYMENT_CRYPTOBOT_ASSET', 'Asset')}: "
                f"{html.escape(str(payment.asset))}"
            )
        if getattr(payment, "amount", None):
            lines.append(
                f"💱 {texts.t('ADMIN_PAYMENT_CRYPTOBOT_AMOUNT', 'Invoice amount')}: "
                f"{html.escape(str(payment.amount))}"
            )
        if getattr(payment, "paid_at", None):
            paid_at = format_datetime(payment.paid_at)
            lines.append(
                f"✅ {texts.t('ADMIN_PAYMENT_CRYPTOBOT_PAID_AT', 'Paid at')}: {paid_at}"
            )

    if record.method == PaymentMethod.TELEGRAM_STARS:
        if getattr(payment, "completed_at", None):
            completed_at = format_datetime(payment.completed_at)
            lines.append(
                f"✅ {texts.t('ADMIN_PAYMENT_STARS_COMPLETED_AT', 'Completed at')}: {completed_at}"
            )
        if record.amount_kopeks:
            stars_amount = settings.rubles_to_stars(record.amount_kopeks / 100)
            lines.append(
                f"⭐ {texts.t('ADMIN_PAYMENT_STARS_AMOUNT', 'Stars used')}: {stars_amount}"
            )
