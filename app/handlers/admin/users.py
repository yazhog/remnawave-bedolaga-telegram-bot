...
def _evaluate_template_restrictions(template, subscription: Optional[Subscription]) -> Tuple[bool, Optional[str]]:
    if template.offer_type == "extend_discount" and subscription and subscription.is_trial:
        return False, "trial_blocked"

    if (
        template.offer_type == "purchase_discount"
        and subscription
        and subscription.is_active
        and not subscription.is_trial
    ):
        return False, "active_paid_blocked"

    return True, None


def _format_template_block_reason(texts, template, reason_code: str) -> str:
    if reason_code == "trial_blocked":
        return texts.t(
            "ADMIN_USER_PROMO_TEMPLATE_BLOCKED_TRIAL",
            "⚠️ Шаблон «{name}» недоступен: нельзя продлевать тестовую подписку.",
        ).format(name=template.name)

    if reason_code == "active_paid_blocked":
        return texts.t(
            "ADMIN_USER_PROMO_TEMPLATE_BLOCKED_ACTIVE",
            "⚠️ Шаблон «{name}» недоступен: у пользователя активная платная подписка.",
        ).format(name=template.name)

    return ""


async def _load_target_user_profile(db: AsyncSession, user_id: int) -> Tuple[Optional[User], Optional[Subscription]]:
    user_service = UserService()
    profile = await user_service.get_user_profile(db, user_id)
    if not profile:
        return None, None
    ...
    return target_user, subscription


def _build_user_promo_preview_keyboard(texts, preview_data: dict) -> InlineKeyboardMarkup:
    ...
    inline_keyboard = [
        [
            InlineKeyboardButton(
                text=texts.t("ADMIN_USER_PROMO_TEMPLATE_EDIT_MESSAGE", "✏️ Изменить текст"),
                callback_data=f"admin_user_promo_edit_message_{user_id}_{template_id}",
            )
        ],
        [
            InlineKeyboardButton(
                text=texts.t("ADMIN_USER_PROMO_TEMPLATE_EDIT_BUTTON", "🔘 Изменить кнопку"),
                callback_data=f"admin_user_promo_edit_button_{user_id}_{template_id}",
            )
        ],
        [
            InlineKeyboardButton(
                text=texts.t("ADMIN_USER_PROMO_TEMPLATE_CONFIRM_SEND", "✅ Отправить"),
...
def _build_user_promo_preview_text(texts, template, target_user: User, preview_data: dict) -> str:
    user_language = preview_data.get("user_language") or target_user.language or "-"
    message_text = preview_data.get("message_text", "")
    button_text = preview_data.get("button_text", template.button_text)

    lines = [
        texts.t(
            "ADMIN_USER_PROMO_TEMPLATE_PREVIEW_TITLE",
            "📄 <b>Предпросмотр «{name}»</b>",
        ).format(name=template.name),
        texts.t(
            "ADMIN_USER_PROMO_TEMPLATE_PREVIEW_RECIPIENT",
            "👤 Получатель: {user}",
        ).format(user=target_user.full_name),
        texts.t(
            "ADMIN_USER_PROMO_TEMPLATE_PREVIEW_LANGUAGE",
            "🌐 Язык пользователя: {language}",
        ).format(language=user_language),
        "",
        message_text,
        "",
        texts.t(
            ...
            "🔘 Текст кнопки: {text}",
        ).format(text=html.escape(button_text)),
        "",
        texts.t(
            "ADMIN_USER_PROMO_TEMPLATE_PREVIEW_HINT",
            "Отредактируйте текст и кнопку перед отправкой.",
        ),
    ]
...
async def _refresh_user_promo_preview(
    bot,
    db: AsyncSession,
    preview_data: dict,
    admin_language: str,
) -> bool:
    chat_id = preview_data.get("chat_id")
    message_id = preview_data.get("message_id")
    user_id = preview_data.get("user_id")
    template_id = preview_data.get("template_id")

    if not all([chat_id, message_id, user_id, template_id]):
        return False

    target_user, _ = await _load_target_user_profile(db, user_id)
    if not target_user:
        return False
    ...
    preview_text = _build_user_promo_preview_text(texts, template, target_user, preview_data)
    keyboard = _build_user_promo_preview_keyboard(texts, preview_data)
...
async def _clear_user_promo_preview_state(state: FSMContext) -> None:
    data = await state.get_data()
    if "user_promo_preview" in data:
        data.pop("user_promo_preview", None)
...
async def show_user_promo_templates(
    ...
    target_user, subscription = await _load_target_user_profile(db, user_id)
    if not target_user:
        await callback.answer("❌ Пользователь не найден", show_alert=True)
        return
    ...
    await _clear_user_promo_preview_state(state)
    ...
    message_lines = [
        ...
        texts.t(
            "ADMIN_USER_PROMO_TEMPLATES_EDIT_HINT",
            "После выбора вы сможете отредактировать сообщение перед отправкой.",
        ),
    ]
    ...
    for template in active_templates:
        allowed, reason_code = _evaluate_template_restrictions(template, subscription)
        if allowed:
            allowed_templates.append(template)
        elif reason_code:
            blocked_messages.append(_format_template_block_reason(texts, template, reason_code))
    ...
    if not allowed_templates:
        ...
        if blocked_messages:
            message_lines.extend(["", *blocked_messages])
        ...
        keyboard_rows.append(
            [
                InlineKeyboardButton(
                    ...
                    callback_data=f"admin_user_promo_prepare_{target_user.id}_{template.id}",
                )
            ]
        )
...
@admin_required
@error_handler
async def prepare_user_promo_template(
    callback: types.CallbackQuery,
    db_user: User,
    db: AsyncSession,
    state: FSMContext,
):
    prefix = "admin_user_promo_prepare_"
    ...
    if not allowed:
        texts = get_texts(db_user.language)
        reason = _format_template_block_reason(texts, template, reason_code or "")
        await callback.answer(reason or "❌ Шаблон недоступен", show_alert=True)
        return
    ...
    initial_message = _render_promo_template_text(
        template,
        user_language,
    ...
    await state.update_data(user_promo_preview=preview_data)
    await state.set_state(None)
...
@admin_required
@error_handler
async def confirm_user_promo_template_send(
    callback: types.CallbackQuery,
    db_user: User,
    db: AsyncSession,
    state: FSMContext,
    ...
    data = await state.get_data()
    preview_data = data.get("user_promo_preview") if data else None
    if not preview_data or preview_data.get("user_id") != user_id or preview_data.get("template_id") != template_id:
    ...
    message_text = preview_data.get("message_text") or _render_promo_template_text(
        template,
        preview_data.get("user_language") or target_user.language or db_user.language,
        server_name=squad_name,
    )
    button_text = preview_data.get("button_text") or template.button_text
    ...
    user_language = preview_data.get("user_language") or target_user.language or db_user.language
...
@admin_required
@error_handler
async def prompt_user_promo_message_edit(
    callback: types.CallbackQuery,
    db_user: User,
    db: AsyncSession,
    state: FSMContext,
    ...
    _ = db
    prefix = "admin_user_promo_edit_message_"
    if not callback.data.startswith(prefix):
        await callback.answer("❌ Некорректные данные", show_alert=True)
        return
    ...
    await state.set_state(AdminStates.editing_user_promo_message)

    await callback.message.answer(
        texts.t(
            "ADMIN_USER_PROMO_TEMPLATE_EDIT_MESSAGE_PROMPT",
            "✏️ Отправьте новый текст сообщения. Используйте /cancel для отмены.",
        )
    )
    await callback.answer()
...
@admin_required
@error_handler
async def prompt_user_promo_button_edit(
    callback: types.CallbackQuery,
    db_user: User,
    db: AsyncSession,
    state: FSMContext,
    ...
    _ = db
    prefix = "admin_user_promo_edit_button_"
    if not callback.data.startswith(prefix):
        await callback.answer("❌ Некорректные данные", show_alert=True)
        return
    ...
    await state.set_state(AdminStates.editing_user_promo_button)

    await callback.message.answer(
        texts.t(
            "ADMIN_USER_PROMO_TEMPLATE_EDIT_BUTTON_PROMPT",
            "🔘 Отправьте новый текст кнопки (до 64 символов). Используйте /cancel для отмены.",
        )
    )
    await callback.answer()
...
@admin_required
@error_handler
async def process_user_promo_message_edit(
    message: types.Message,
    db_user: User,
    state: FSMContext,
    db: AsyncSession,
    ...
    if not preview_data:
        await message.answer(texts.t("ADMIN_USER_PROMO_TEMPLATE_PREVIEW_NO_DATA", "❌ Нет данных для редактирования."))
        await state.set_state(None)
        return
    ...
    if new_text.lower() == "/cancel":
        ...
        await _refresh_user_promo_preview(message.bot, db, preview_data, db_user.language)
        return

    if not new_text:
        await message.answer(
            texts.t(
                "ADMIN_USER_PROMO_TEMPLATE_EDIT_EMPTY",
    ...
    await message.answer(
        texts.t(
            "ADMIN_USER_PROMO_TEMPLATE_EDIT_SAVED",
            "Текст обновлён.",
        )
    )
...
@admin_required
@error_handler
async def process_user_promo_button_edit(
    message: types.Message,
    db_user: User,
    state: FSMContext,
    db: AsyncSession,
    ...
    if len(new_text) > 64:
        await message.answer(
            texts.t(
                "ADMIN_USER_PROMO_TEMPLATE_EDIT_BUTTON_TOO_LONG",
                "Текст кнопки не может быть длиннее 64 символов.",
            )
        )
        return

    preview_data["button_text"] = new_text
    await state.update_data(user_promo_preview=preview_data)
    await state.set_state(None)
...
def register_handlers(dp: Dispatcher):
    ...
    dp.callback_query.register(
        prepare_user_promo_template,
        F.data.startswith("admin_user_promo_prepare_")
    )

    dp.callback_query.register(
        confirm_user_promo_template_send,
        F.data.startswith("admin_user_promo_confirm_")
    )

    dp.callback_query.register(
        prompt_user_promo_message_edit,
        F.data.startswith("admin_user_promo_edit_message_")
    )

    dp.callback_query.register(
        prompt_user_promo_button_edit,
        F.data.startswith("admin_user_promo_edit_button_")
    )
    ...
    dp.message.register(
        process_user_promo_message_edit,
        AdminStates.editing_user_promo_message
    )

    dp.message.register(
        process_user_promo_button_edit,
        AdminStates.editing_user_promo_button
    )
