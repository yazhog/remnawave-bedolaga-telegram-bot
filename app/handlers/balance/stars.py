    ...
    try:
        ...
        if prompt_message_id and prompt_chat_id:
            try:
                ...
                logger.debug("Не удалось удалить сообщение с запросом суммы Stars", exc_info=True)

        try:
            await message.delete()
        except Exception:
            logger.debug("Не удалось удалить сообщение пользователя с суммой Stars", exc_info=True)

        try:
            from app.handlers.stars_payment_cleanup import remember_stars_invoice_message

            remember_stars_invoice_message(
                payload=f"balance_{db_user.id}_{amount_kopeks}",
                chat_id=message.chat.id,
                message_id=invoice_message.message_id,
            )
        except Exception:
            logger.debug("Не удалось сохранить данные сообщения инвойса Stars", exc_info=True)
