...
@pytest.mark.anyio("asyncio")
async def test_create_payment_link_stars_generates_unique_payload(monkeypatch):
    ...
    class DummyPaymentService:
        ...
        async def create_stars_invoice(self, amount_kopeks, description, payload):
            captured_payloads.append(payload)
            return 'https://stars'

    async def fake_resolve_user(db, init_data):
        return types.SimpleNamespace(id=42, language='ru'), {}

    monkeypatch.setattr(miniapp, 'Bot', DummyBot)
    monkeypatch.setattr(miniapp, 'PaymentService', lambda bot=None: DummyPaymentService(bot))
    monkeypatch.setattr(miniapp, '_resolve_user_from_init_data', fake_resolve_user)

    request = MiniAppPaymentCreateRequest(
        initData='data',
        method='stars',
        amountKopeks=1000,
    )

    response1 = await miniapp.create_payment_link(request, db=types.SimpleNamespace())
    response2 = await miniapp.create_payment_link(request, db=types.SimpleNamespace())

    assert response1.extra['invoice_payload'] != response2.extra['invoice_payload']
    assert response1.extra['invoice_payload'].startswith('balance_42_1000_')
    assert response2.extra['invoice_payload'].startswith('balance_42_1000_')
    assert captured_payloads == [response1.extra['invoice_payload'], response2.extra['invoice_payload']]
...
@pytest.mark.anyio("asyncio")
async def test_resolve_stars_status_uses_payload_filter(monkeypatch):
    queried_payloads = []

    async def fake_find_recent_deposit(db, **kwargs):
        queried_payloads.append(kwargs.get('invoice_payload'))
        return None

    monkeypatch.setattr(miniapp, '_find_recent_deposit', fake_find_recent_deposit)
    ...
    query = MiniAppPaymentStatusQuery(
        ...
        amountKopeks=5000,
        startedAt='2024-01-01T00:00:00Z',
        payload='balance_7_5000_unique',
    )

    result = await miniapp._resolve_stars_payment_status(db=None, user=user, query=query)

    assert result.status == 'pending'
    assert queried_payloads == ['balance_7_5000_unique']
